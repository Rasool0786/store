...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        all_categories = [
            CategoryFactory(top_product=None) for _ in range(NUM_CATEGORIES)
        ]
        print("DONE")
        ...
        for _ in range(NUM_PRODUCTS):
            ...
            new_product.datetime_created = datetime(
                random.randrange(2019, 2023),
                random.randint(1, 12),
                random.randint(1, 12),
                tzinfo=timezone.utc,
            )
            new_product.datetime_modified = new_product.datetime_created + timedelta(
        ...
        all_customers = [
            (
                ...
                else CustomerFactory(birth_date=None)
            )
        ...
        all_orders = [
            OrderFactory(customer_id=random.choice(all_customers).id)
            for _ in range(NUM_ORDERS)
        ]
        for order in all_orders:
            order.datetime_created = datetime(
                random.randrange(2019, 2023),
                random.randint(1, 12),
                random.randint(1, 12),
                tzinfo=timezone.utc,
